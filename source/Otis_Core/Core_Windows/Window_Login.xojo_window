...
Begin Window Window_Login
   ...
   Begin TextField TextField_Password
      ...
      Width           =   199
   End
   Begin TextField TextField_serverAddress
      AcceptTabs      =   False
      Alignment       =   0
      AutoDeactivate  =   True
      AutomaticallyCheckSpelling=   False
      ...
      Width           =   199
   End
   Begin TextField TextField_Port
      AcceptTabs      =   False
      Alignment       =   0
      AutoDeactivate  =   True
      AutomaticallyCheckSpelling=   False
      ...
      Width           =   199
   End
   Begin TextField TextField_Database
      AcceptTabs      =   False
      Alignment       =   0
      AutoDeactivate  =   True
      AutomaticallyCheckSpelling=   False
   ...
   Begin Label Label_Password
      ...
      Width           =   100
   End
   Begin Label Label_serverAddress
      AutoDeactivate  =   True
      Bold            =   False
      DataField       =   ""
      DataSource      =   ""
   ...
   Begin CheckBox CheckBox_saveUsername
      ...
      Width           =   116
   End
   Begin CheckBox CheckBox_savePassword
      AutoDeactivate  =   True
      Bold            =   False
      ...
      Width           =   120
   End
   Begin ImageWell ImageWell_Otis
      AutoDeactivate  =   True
      Enabled         =   True
      Height          =   243
      HelpTag         =   ""
...
#tag Events PushButton_Ok
	#tag Event
		Sub Action()
		  ...
		  // Open our progress window
		  
		  saveUN = CheckBox_saveUsername.Value
		  savePass = CheckBox_savePassword.Value
		  
		  
		  
		  // Update our login information from TextFields
		  dbUsername = TextField_Username.Text.ToText
		  dbPassword = TextField_Password.Text.ToText
		  dbHost = TextField_serverAddress.Text.ToText
		  dbPort = val( TextField_Port.Text )
		  dbName = TextField_Database.Text.ToText
		  
		  
		  // Save the Username is instructed to
		  If saveUN = True then
		    saveUserName
		  End If
		  
		  // Save the password if instructed to 
		  If savePass = True Then
		    savePassword
		  End If 
		  
		  // Lets Login
		  If dbUsername <> "" And dbPassword <> "" And dbHost <> "" And dbPort <> 0 And dbName <> "" Then
		    If login Then  'Login Successful
		      me.Window.Close
		      MainWindow.myOpen
		    Else
		      logErrorMessage( 4, "DBase", "Could Not Log In" )
		    End If
		  End If
...
#tag Events PushButton_Cancel
	...
	#tag EndEvent
#tag EndEvents
#tag Events CheckBox_saveUsername
	#tag Event
		Sub Action()
		  If me.Value = True Then
		    CheckBox_savePassword.Enabled = True
		  Else
		    CheckBox_savePassword.Enabled = False
		  End If
		End Sub
	#tag EndEvent
#tag EndEvents
